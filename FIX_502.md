...
✅ Admin user created
🔐 Admin user 'admin' is ready (membership: pro)
```
...
```bash
cd /root/montanablotter
ls -l config.py  # Make sure the loader exists
ls -l .env       # Ensure your environment file is present

# If missing, upload config.py and create .env from .env.example
```
...
ExecStart=/opt/montanablotter/venv/bin/gunicorn -w 3 -b 127.0.0.1:5000 app:app
Restart=always
...
- **Username:** admin
- **Password:** the password you set with `seed_admin.py`
