...
COUNTY_DIRECTORY = {
    ...
    'garfield': {'name': 'Garfield', 'roster_url': 'https://www.garcosheriff.com/PDFs/current_inmates.pdf', 'phone': '406-557-2540', 'has_online_roster': True},
...
# ---------------------------------------------------------------------------
# /learn — Know Your Rights course (7 chapters, ~13.5k words, free, public)
# /learn/<slug> — single-chapter reader; markdown rendered via the site-wide
#                 markdown filter + sanitizer.
# /how-to-use — the previous /learn page (product-help guide for using
#               Montana Blotter itself) is preserved here so existing
#               links from emails, social, and bookmarks still resolve.
# ---------------------------------------------------------------------------
import os as _os_learn
import re as _re_learn
from pathlib import Path as _Path_learn

_LEARN_CONTENT_DIR = _Path_learn(_os_learn.path.dirname(_os_learn.path.abspath(__file__))) / 'app_content' / 'learn'

_LEARN_CHAPTER_META = [
    {'number': 1, 'slug': '01-encounters',          'title': 'When a police officer walks up to you',
...
def _learn_chapter_word_count(text):
    if not text:
        return 0
    return len(_re_learn.findall(r'\b\w+\b', text))
...
def _learn_load_chapter(slug):
    ...
    h1 = _re_learn.match(r'^\s*#\s+(.+?)\s*\n', raw)
    body = raw[h1.end():].lstrip('\n') if h1 else raw
    ...
    return {
        ...
        'word_count': wc,
        'read_minutes': max(1, round(wc / 220)),
    }
...
def _learn_load_all_chapters():
    ...
    for meta in _LEARN_CHAPTER_META:
        ...
        if ch is None:
            continue
        ch.update({'number': meta['number'], 'title': meta['title'], 'summary': meta['summary']})
        out.append(ch)
    return out
...
def how_to_use():
    """The previous /learn content (product-help guide) preserved at its new URL."""
    return render_template(
        'how_to_use.html',
        active_nav='learn',
        ...
        canonical_url=f'{BASE_URL}/how-to-use',
...
def learn_index():
    ...
    return render_template(
        ...
        active_nav='learn',
        page_title='Know Your Rights in Montana',
        meta_description='A free, plain-English course from Montana Blotter on Montana law — written for residents, drivers, renters, outdoorspeople, and parents. Every claim links to the Montana Code Annotated.',
        canonical_url=f'{BASE_URL}/learn',
        og_title='Know Your Rights in Montana | Montana Blotter',
        og_description='A free, plain-English course on Montana law. Every claim links to the Montana Code Annotated.',
    )
...
def learn_chapter(slug):
    ...
    return render_template(
        ...
        meta_description=chapter['summary'],
        canonical_url=f'{BASE_URL}/learn/{slug}',
        og_title=f"{chapter['title']} | Know Your Rights in Montana",
        og_description=chapter['summary'],
    )
