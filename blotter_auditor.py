...
Return ONLY valid JSON — no markdown fences, no extra keys — with this structure:
{
  ...
  "seo_title": "60-65 char title: county + incident type + date, no site name suffix"
}
"""
...
def _save_audit_result(conn: sqlite3.Connection, result: AuditResult) -> None:
    ...
    conn.execute(
        ...
        UPDATE posts
           SET audit_status     = ?,
               ...
               meta_description = ?,
               seo_title        = ?,
               seo_slug         = CASE WHEN ? != '' THEN ? ELSE seo_slug END,
               audited_at       = ?,
    ...
    if status == "manual_review":
        high_flags = [f for f in result.pii_flags if f.severity == "high"]
        logger.critical(
            "POST %s BLOCKED — %d HIGH-severity PII flag(s) detected: %s",
            result.post_id,
            len(high_flags),
            ", ".join(f.pii_type for f in high_flags),
        )
        try:
            conn.execute(
                """
                INSERT INTO audit_logs (action, target_type, target_id, metadata_json)
                VALUES ('pii_high_severity_blocked', 'post', ?, ?)
                """,
                (
                    str(result.post_id),
                    json.dumps({
                        "flag_count": len(high_flags),
                        "types": [f.pii_type for f in high_flags],
                        "audit_issues": result.raw_issues,
                    }),
                ),
            )
        except Exception as exc:
            logger.error("Failed to write PII alert to audit_logs: %s", exc)
    conn.commit()
...
def audit_post(
    ...
    seo_title      = claude_result.get("seo_title", "")
    seo_slug       = ""
    if seo_title and post_id:
        import re as _re
        slug_base = _re.sub(r'[^a-z0-9]+', '-', seo_title.lower().strip()).strip('-')[:60]
        seo_slug  = f"{slug_base}-{post_id}"
...
_SEO_BACKFILL_PROMPT = """\
You are an SEO specialist for Montana Blotter, a Montana public records news site.

Given this post metadata, return ONLY valid JSON with exactly these two keys:
...
def _build_seo_prompt(post: sqlite3.Row) -> str:
    summary_excerpt = (post["summary"] or "")[:300].strip()
    return _SEO_BACKFILL_PROMPT.format(
        title=post["title"] or "",
        county=post["county"] or "",
        agency_name=post["agency_name"] or "",
        incident_date=post["incident_date"] or "",
...
def _call_groq_seo(prompt: str, post_id: int) -> dict:
    """Call Groq (llama3-8b-8192) as a free fallback for SEO title generation."""
    ...
    try:
        ...
        return _parse_claude_json(resp.choices[0].message.content.strip())
    except Exception as e:
        logger.error(f"Groq SEO fallback error for post #{post_id}: {e}")
        return {}


def _call_claude_seo(client, post: sqlite3.Row) -> dict:
    prompt = _build_seo_prompt(post)
    # Try Anthropic first
    if client:
        try:
            message = client.messages.create(
            ...
            return _parse_claude_json(message.content[0].text.strip())
        except Exception as e:
            logger.warning(f"Anthropic SEO error for post #{post['id']}, trying Groq: {e}")
    # Fall back to Groq
    result = _call_groq_seo(prompt, post["id"])
    if result:
        return result
    logger.error(f"SEO backfill: all providers failed for post #{post['id']}")
    return {}


def backfill_seo(db_path: Optional[str] = None, batch_size: int = 50, force: bool = False) -> int:
    """
    Fill seo_title / meta_description / seo_slug on clean posts that are missing them.
    Does NOT touch audit_status, pii_flags, or summary.
    force=True re-processes posts that already have seo_title (use after API credits restored).
    ...
    where_clause = "audit_status = 'clean'" if force else "audit_status = 'clean' AND (seo_title IS NULL OR seo_title = '')"
    posts = conn.execute(
        f"""
        SELECT id, title, county, agency_name, incident_date, summary
        FROM posts
        WHERE {where_clause}
        ORDER BY created_at DESC
        LIMIT ?
    ...
    for post in posts:
        ...
        meta_desc = ""
        if client:
            result = _call_claude_seo(client, post)
            seo_title = result.get("seo_title", "")
            meta_desc = result.get("meta_description", "")

        if not seo_title:
            # Fallback: derive from title + county
            seo_title = f"{post['county']} County {post['incident_date'] or ''} — Montana Blotter"
            seo_title = seo_title[:65]

        slug_base = _re.sub(r'[^a-z0-9]+', '-', seo_title.lower().strip()).strip('-')[:60]
        seo_slug = f"{slug_base}-{post['id']}"

        conn.execute(
            """
            UPDATE posts
               SET seo_title = ?,
                   seo_slug  = CASE WHEN seo_slug IS NULL OR seo_slug = '' THEN ? ELSE seo_slug END,
                   meta_description = CASE WHEN (meta_description IS NULL OR meta_description = '') AND ? != '' THEN ? ELSE meta_description END
             WHERE id = ?
            """,
            (seo_title, seo_slug, meta_desc, meta_desc, post["id"]),
        )
        conn.commit()
        updated += 1
        logger.info(f"SEO backfill post #{post['id']}: {seo_title!r}")
        print(f"  #{post['id']:5d}  {seo_title[:65]}")
...
def print_audit_report(result: AuditResult, post_title: str = "") -> None:
    ...
    if result.seo_title:
        ...
        title_color = C.GREEN if 55 <= title_len <= 65 else C.YELLOW
        print(_colorize(f"\n  SEO TITLE ({title_len} chars):", C.BOLD))
        print(f"  {_colorize(result.seo_title, title_color)}")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    group.add_argument(
        "--backfill-seo", action="store_true",
        help="Fill seo_title/seo_slug on clean posts missing them (safe — does not change audit_status)",
    )
    group.add_argument(
        "--backfill-seo-force", action="store_true",
        help="Re-run SEO backfill on ALL clean posts, overwriting existing seo_title (use after topping up API credits)",
    )
    group.add_argument(
    ...
    p.add_argument(
        ...
        help="Max posts to process per run for --backfill-seo (default: 50)",
    )
    p.add_argument(
...
def main() -> None:
    ...
    elif getattr(args, "backfill_seo", False):
        count = backfill_seo(db_path, batch_size=args.batch)
        print(f"\nSEO backfill complete: {count} post(s) updated.")
        sys.exit(0)

    elif getattr(args, "backfill_seo_force", False):
        count = backfill_seo(db_path, batch_size=args.batch, force=True)
        print(f"\nSEO backfill (force) complete: {count} post(s) updated.")
        sys.exit(0)
