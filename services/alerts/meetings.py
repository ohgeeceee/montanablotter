#!/usr/bin/env python3
"""
Admin alerts for stale or failing public meetings sources.
"""
...
import config
from services.alerts.ingestion import _app_setting_int
from services.alerts.legacy import collect_alert_recipients, send_plaintext_email as _send_plaintext_email
...
def _active_alerts(conn: sqlite3.Connection) -> dict[tuple[str, str], dict]:
    ...
    for source in context['sources']:
        ...
        alerts[(source['slug'], alert_kind)] = {
            ...
            'meeting_scope': source['meeting_scope'],
            'location_name': source['location_display_name'] or source['city_name'] or source['county_name'] or '',
...
def _format_active_body(alerts: list[dict], heading: str) -> str:
    lines = [
        heading,
        '',
        f"Admin page: {config.BASE_URL.rstrip('/')}/admin/operations/meetings",
        '',
    ]
    for alert in alerts:
        lines.extend([
...
def _format_resolved_body(alerts: list[sqlite3.Row]) -> str:
    lines = [
        'The following meetings source alerts resolved:',
        '',
        f"Admin page: {config.BASE_URL.rstrip('/')}/admin/operations/meetings",
        '',
    ]
...
def run(*, dry_run: bool = False, force_reminder: bool = False) -> int:
    conn = _connect()
    repeat_hours = max(1, _app_setting_int(conn, 'meeting_alert_repeat_hours', 24))
    ...
    for key, alert in active.items():
        ...
        if row is None:
            ...
            if not dry_run:
                conn.execute(
                    '''
                    INSERT INTO meeting_source_alerts (
    ...
    if new_alerts or reminder_alerts:
        subject = f"[Montana Blotter] Meetings source alerts: {len(new_alerts) + len(reminder_alerts)} source(s)"
        body_parts = []
        if new_alerts:
            body_parts.append(_format_active_body(new_alerts, 'New meetings source alerts detected:'))
        if reminder_alerts:
            body_parts.append(_format_active_body(reminder_alerts, 'Ongoing meetings source alerts still active:'))
    ...
    if resolved_alerts:
        sent_resolved = True if dry_run else _send_plaintext_email(
            recipients,
            f"[Montana Blotter] Meetings source recovered: {len(resolved_alerts)} source(s)",
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Send admin alerts for stale or failing meetings sources.')
