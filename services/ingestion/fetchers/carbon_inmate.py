...
carbon_inmate.py
================
Fetches Carbon County (MT) inmate roster via Zuercher portal.

The portal at https://carbon-so-mt.zuercherportal.com/#/inmates was reported
as not exposing ``/api/public/inmate/criteria``.  This fetcher reads the
portal's homepage JS to discover the real criteria endpoint and falls back to a
Zuercher-style guest-link fetch if the portal is JS-only.
"""
...
BASE_URL = "https://carbon-so-mt.zuercherportal.com"
...
def _discover_criteria_url(session: requests.Session) -> str:
    try:
        ...
        logger.warning("Carbon portal fetch failed: %s", exc)
...
def _parse_payload(payload: dict | list) -> list[JailBookingRecord]:
    ...
    for inmate in inmates:
        ...
        if isinstance(charges_raw, str):
            ...
            ) or "Charge details available on the official Carbon County portal."
        else:
            charges_summary = "Charge details available on the official Carbon County portal."

        source_record_id = f"carbon:{person_name.lower().replace(' ', '-')}:{inmate_id}"
    ...
    logger.info("Carbon: parsed %d records", len(records))
    return records


def fetch_carbon_bookings(source_url: str | None = None) -> list[JailBookingRecord]:
    ...
    logger.info("Carbon criteria URL: %s", criteria_url)
    try:
        resp = session.get(criteria_url, timeout=30)
        resp.raise_for_status()
        payload = resp.json()
    except Exception as exc:
        logger.warning("Carbon criteria fetch failed: %s", exc)
        return []
