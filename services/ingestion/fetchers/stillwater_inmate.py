#!/usr/bin/env python3
"""
stillwater_inmate.py
====================
Fetches Stillwater County (MT) inmate roster via Zuercher portal.

The portal at https://stillwater-so-mt.zuercherportal.com/#/inmates returns a
404 on ``/api/public/inmate/criteria``.  This fetcher reads the portal's JS to
discover the real endpoint, then hits it for the inmate list.
"""
...
BASE_URL = "https://stillwater-so-mt.zuercherportal.com"
...
def _discover_criteria_url(session: requests.Session) -> str:
    try:
        ...
        logger.warning("Stillwater portal fetch failed: %s", exc)
...
def _parse_payload(payload: dict | list) -> list[JailBookingRecord]:
    ...
    for inmate in inmates:
        ...
        if isinstance(charges_raw, str):
            ...
            ) or "Charge details available on the official Stillwater County portal."
        else:
            charges_summary = "Charge details available on the official Stillwater County portal."

        source_record_id = f"stillwater:{person_name.lower().replace(' ', '-')}:{inmate_id}"
    ...
    logger.info("Stillwater: parsed %d records", len(records))
    return records


def fetch_stillwater_bookings(source_url: str | None = None) -> list[JailBookingRecord]:
    ...
    logger.info("Stillwater criteria URL: %s", criteria_url)
    try:
        resp = session.get(criteria_url, timeout=30)
        resp.raise_for_status()
        payload = resp.json()
    except Exception as exc:
        logger.warning("Stillwater criteria fetch failed: %s", exc)
        return []
...
if __name__ == "__main__":
    ...
    recs = fetch_stillwater_bookings()
