...
wheatland_inmate.py
===================
Fetches Wheatland County (MT) inmate roster via Zuercher portal.

The portal at https://wheatland-so-mt.zuercherportal.com/#/inmates returns a
404 on ``/api/public/inmate/criteria``.  This fetcher reads the portal's JS to
discover the real endpoint.
"""
...
BASE_URL = "https://wheatland-so-mt.zuercherportal.com"
...
def _discover_criteria_url(session: requests.Session) -> str:
    try:
        ...
        logger.warning("Wheatland portal fetch failed: %s", exc)
...
def _parse_payload(payload: dict | list) -> list[JailBookingRecord]:
    ...
    for inmate in inmates:
        ...
        if isinstance(charges_raw, str):
            ...
            ) or "Charge details available on the official Wheatland County portal."
        else:
            charges_summary = "Charge details available on the official Wheatland County portal."

        source_record_id = f"wheatland:{person_name.lower().replace(' ', '-')}:{inmate_id}"
    ...
    logger.info("Wheatland: parsed %d records", len(records))
    return records


def fetch_wheatland_bookings(source_url: str | None = None) -> list[JailBookingRecord]:
    ...
    logger.info("Wheatland criteria URL: %s", criteria_url)
    try:
        resp = session.get(criteria_url, timeout=30)
        resp.raise_for_status()
        payload = resp.json()
    except Exception as exc:
        logger.warning("Wheatland criteria fetch failed: %s", exc)
        return []
